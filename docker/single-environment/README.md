...
4. Preview what will be synced:
   ...
   ```bash
   docker compose run --rm ccfm --config /ccfm.yaml deploy --directory /docs --plan
   ```

5. Deploy to Confluence:

   ```bash
   docker compose run --rm ccfm --config /ccfm.yaml deploy --directory /docs --changed-only --archive-orphans
   ```
